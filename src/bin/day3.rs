...
fn part1(input: &str, right: usize, down: usize) -> Result<u64, Box<dyn Error>> {
    ...
    while y < board.len() {
        let curr = *board[y].iter().cycle().nth(x).expect("This should never happen");
        if curr == '#' {
            count += 1;
        }
...
fn part2(input : &str) -> Result<(), Box<dyn Error>> {
    let answer = part1(input, 1, 1)? * part1(input, 3, 1)? * part1(input, 5, 1)?
        * part1(input, 7, 1)? * part1(input, 1, 2)?;
